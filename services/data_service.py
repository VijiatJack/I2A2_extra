...
class DataService:
    ...
    def detect_data_type(self, data: Optional[pd.DataFrame] = None) -> str:
        """
        Detect the type of dataset (fraud detection, general, etc.).
        ...
        if data is not None:
            ...
            raise ValueError("No data available for type detection")
        ...
        financial_indicators = ['amount', 'price', 'cost', 'revenue', 'profit']
        if any(indicator in col.lower() for col in df.columns for indicator in financial_indicators):
            return 'financial'
